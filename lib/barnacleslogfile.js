...
const DYNAMB_PROPERTIES = [ 'acceleration', 'batteryPercentage',
                            'batteryVoltage', 'interactionDigest', 'nearest',
                            'relativeHumidity', 'temperatue', 'txCount',
                            'uptime' ];
...
function createLogfile(instance, event, options) {
  ...
  let logfile = {
      ...
      header: createLogfileHeader(event, delimiter),
      delimiter: delimiter,
      lastRotationTime: 0
  }
 ...
 * @param {BarnaclesLogfile} instance The BarnaclesLogfile instance.
 */
function createLogfileHeader(event, delimiter) {
  ...
  switch(event) {
    ...
    case 'dynamb':
      ...
      DYNAMB_PROPERTIES.forEach(function(property) {
