...
const DYNAMB_PROPERTIES = [ 'acceleration', 'amperage', 'amperages',
                            'angleOfRotation', 'batteryPercentage',
                            'batteryVoltage', 'carbonDioxideConcentration',
                            'distance', 'elevation', 'heading', 'heartRate',
                            'illuminance', 'interactionDigest',
                            'isButtonPressed', 'isContactDetected', 'isHealthy',
                            'isLiquidDetected', 'isMotionDetected', 'languages',
                            'levelPercentage',  'magneticField', 'nearest',
                            'numberOfOccupants', 'passageCounts', 'pH', 
                            'position', 'pressure', 'pressures', 
                            'relativeHumidity', 'speed', 'temperature',
                            'temperatures', 'txCount', 'unicodeCodePoints',
                            'uptime', 'voltage', 'voltages' ];
 ...
 */
function manageRotation(logfile, callback) {
  ...
  if(isDueForRotation) {
    ...
    fs.appendFile(logfile.filePath, logfile.header, (error) => {
      handleError(error);
      return callback();
    });
  }
